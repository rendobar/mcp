...
3. If the file is in a bucket the user connected on Rendobar, pass storage://<id>/<path> as the input. list_storage gives the ids and list_storage_files the paths. To write the output back into a bucket, add destinations to submit_job.
4. For expensive jobs, call get_account first to confirm the balance covers the cost.
5. After submit_job, call get_job with wait:true — it blocks until the job finishes (up to ~50s, then returns a snapshot; call again to keep waiting). The output URL is on the complete response.
6. If submit_job fails with INVALID_JOB_TYPE, call list_job_types and pick from what it returns.
